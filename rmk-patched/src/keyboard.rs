...
/// Currently held mouse buttons (bitmask), shared with input processors (e.g. the trackball)
/// so that pointer movement keeps held buttons pressed (drag) and disables scroll while a button is held.
pub(crate) static MOUSE_BUTTONS_STATE: core::sync::atomic::AtomicU8 = core::sync::atomic::AtomicU8::new(0u8);

/// Set while the dedicated scroll key (User8) is held. The trackball processor turns
    ...
    Keyboard<'a, ROW, COL, NUM_LAYER, NUM_ENCODER>
{
    ...
    async fn process_action_mouse(&mut self, key: KeyCode, event: KeyboardEvent) {
        if key.is_mouse_key() {
            ...
            // Publish the current mouse button state so input processors (trackball) can
            // keep buttons pressed during movement (drag) and disable scroll while held.
            MOUSE_BUTTONS_STATE.store(self.mouse_report.buttons, core::sync::atomic::Ordering::Relaxed);
    ...
    async fn process_user(&mut self, key: KeyCode, event: KeyboardEvent) {
        ...
        // Dedicated scroll key: hold to make the trackball scroll. Emits no HID report.
        if key == KeyCode::User8 {
