...
pub struct Pmw3610Processor<'a, const ROW: usize, const COL: usize, const NUM_LAYER: usize, const NUM_ENCODER: usize> {
    ...
    /// Accumulated horizontal movement waiting to become a scroll tick (pan).
    scroll_acc_h: i32,
    ...
    Pmw3610Processor<'a, ROW, COL, NUM_LAYER, NUM_ENCODER>
{
    ...
    async fn generate_report(&self, x: i16, y: i16) {
        // Carry the currently held mouse buttons so movement keeps them pressed (drag).
        let buttons = crate::keyboard::MOUSE_BUTTONS_STATE.load(core::sync::atomic::Ordering::Relaxed);
    ...
    /// Send a scroll report (vertical = wheel, horizontal = pan), carrying any held buttons.
    async fn generate_scroll_report(&self, wheel: i8, pan: i8) {
        let buttons = crate::keyboard::MOUSE_BUTTONS_STATE.load(core::sync::atomic::Ordering::Relaxed);
        let mouse_report = MouseReport {
            buttons,
            x: 0,
